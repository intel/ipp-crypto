...
IF (_IPP32E GE _IPP32E_M7) AND (_IPP32E LT _IPP32E_E9 )
...
;;
SHA512_STEP_2 MACRO regA:REQ,regB:REQ,regC:REQ,regD:REQ,regE:REQ,regF:REQ,regG:REQ,regH:REQ, \
                  regT:REQ,regF1:REQ,regF2:REQ, \
                  memW:REQ, memCNT:REQ
        ;; update H (start)
...
;;
UPDATE_2 MACRO nr:REQ, sig0, sig1, W15, W2
   mov      sig0, [rsp+((nr-15) and 0Fh)*8]  ;; W[j-15]
   mov      sig1, [rsp+((nr-2) and 0Fh)*8]   ;; W[j-2]
   shr      sig0, 7                          ;; LSR(W[j-15], 7)
   shr      sig1, 6                          ;; LSR(W[j-2],  6)

   mov      W15, [rsp+((nr-15) and 0Fh)*8]   ;; W[j-15]
   mov      W2,  [rsp+((nr-2) and 0Fh)*8]    ;; W[j-2]
   ...
   add      sig0, [rsp+((nr-16) and 0Fh)*8]  ;; SIG0 += W[j-16]
   add      sig1, [rsp+((nr-7) and 0Fh)*8]   ;; SIG1 += W[j-7]
   add      sig0, sig1
   mov      [rsp+((nr-16) and 0Fh)*8], sig0
ENDM
...
IPPASM UpdateSHA512 PROC PUBLIC FRAME
      ...
      LOCAL_FRAME = (16*sizeof(qword)+sizeof(qword))
      USES_XMM
      COMP_ABI 4
...
MBS_SHA512  equ   (128)
   ...
   movsxd   rdx, edx
   mov      qword ptr[rsp+16*sizeof(qword)], rdx   ; save length of buffer
...
;;
IF _IPP32E GE _IPP32E_U8
   movdqu   xmm4, oword ptr pByteSwp

   movdqu   xmm0, oword ptr [rsi+0*16]
   pshufb   xmm0, xmm4
   movdqa   oword ptr [rsp+0*16], xmm0
   movdqu   xmm1, oword ptr [rsi+1*16]
   pshufb   xmm1, xmm4
   movdqa   oword ptr [rsp+1*16], xmm1
   movdqu   xmm2, oword ptr [rsi+2*16]
   pshufb   xmm2, xmm4
   movdqa   oword ptr [rsp+2*16], xmm2
   movdqu   xmm3, oword ptr [rsi+3*16]
   pshufb   xmm3, xmm4
   movdqa   oword ptr [rsp+3*16], xmm3

   movdqu   xmm0, oword ptr [rsi+4*16]
   pshufb   xmm0, xmm4
   movdqa   oword ptr [rsp+4*16], xmm0
   movdqu   xmm1, oword ptr [rsi+5*16]
   pshufb   xmm1, xmm4
   movdqa   oword ptr [rsp+5*16], xmm1
   movdqu   xmm2, oword ptr [rsi+6*16]
   pshufb   xmm2, xmm4
   movdqa   oword ptr [rsp+6*16], xmm2
   movdqu   xmm3, oword ptr [rsi+7*16]
   pshufb   xmm3, xmm4
   movdqa   oword ptr [rsp+7*16], xmm3
ELSE
   xor      rcx,rcx
loop1:
...
;;
   ...
   add      rsi, MBS_SHA512
   sub      qword ptr[rsp+16*sizeof(qword)], MBS_SHA512
...
ENDIF    ;; (_IPP32E GE _IPP32E_M7) AND (_IPP32E LT _IPP32E_E9 )
