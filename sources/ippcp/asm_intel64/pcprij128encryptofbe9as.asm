...
IPPASM EncryptOFB_RIJ128_AES_NI PROC PUBLIC FRAME
      USES_GPR    rsi,rdi,r12,r15
      LOCAL_FRAME = (1+1+4+4)*16
      USES_XMM
      COMP_ABI 7
...
locSrc   equ   [rsp+16*6]
   ...
   movdqu   xmm0, oword ptr[rax]       ; get IV
   movdqa   oword ptr [rsp+0*16], xmm0 ; into the stack
...
key_128_s:
   ...
   movdqa     oword ptr[rsp+1*16], xmm0      ; save chipher output

   movdqu      xmm1, oword ptr[rsp+6*16+r11] ; get src blocks from the stack
   pxor        xmm1, xmm0                    ; xor src
   movdqu      oword ptr[rsp+2*16+r11],xmm1  ;and store into the stack

   movdqu      xmm0, oword ptr[rsp+r9]       ; update chiper input (IV)
   ...
   mov      rax, [rsp+ARG_7]           ; IV address
   movdqa   xmm0, oword ptr[rsp]       ; update IV before return
   movdqu   oword ptr[rax], xmm0
...
ALIGN IPP_ALIGN_FACTOR
IPPASM EncryptOFB128_RIJ128_AES_NI PROC PUBLIC FRAME
      USES_GPR    rsi,rdi
      LOCAL_FRAME = 0
      USES_XMM
      COMP_ABI 6
...
key_128_s:
   ...
   pxor        xmm1, xmm0                    ; xor src
   movdqu      oword ptr[rsi],xmm1           ;and store into the dst
   ...
   movdqu   oword ptr[r9], xmm0        ; update IV before return
