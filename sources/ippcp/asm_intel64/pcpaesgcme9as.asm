...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
REFLECTED_IN_OUT     = 1   ; GCM input and output are represented in reflected form naturaly
;USE_BYTE_REFLECTED   = 0   ; set to 1 if input data is byte-reflected


sse_clmul_gcm_2way   MACRO HK,HKforKaratsuba, GH_1,tmpX0_1,tmpX1_1,tmpX2_1, GH_2,tmpX0_2,tmpX1_2,tmpX2_2,tmpX3_2
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
ALIGN IPP_ALIGN_FACTOR
IPPASM AesGcmPrecompute_avx PROC PUBLIC FRAME
      USES_GPR rdi,rsi
      USES_XMM xmm6,xmm7,xmm8,xmm9,xmm10,xmm11,xmm12,xmm13,xmm14,xmm15
      COMP_ABI 2
pPrecomData equ  rdi         ; (rdi) pointer to the reflected multipliers reflect(hkey),(hkey<<1), (hkey^2)<<1, (hkey^4)<<1,
pHKey       equ  rsi         ; (rsi) pointer to the Hkey value

   movdqu   xmm0, oword ptr [pHKey] ;  xmm0 holds HashKey
   pshufb   xmm0, oword ptr SHUF_CONST
  ;movdqu   oword ptr [pPrecomData+sizeof_oword_*0], xmm0
   ...
   movdqu   oword ptr [pPrecomData+sizeof_oword_*0], xmm0
   movdqu   oword ptr [pPrecomData+sizeof_oword_*1], xmm1
   movdqu   oword ptr [pPrecomData+sizeof_oword_*2], xmm2
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
ALIGN IPP_ALIGN_FACTOR
IPPASM AesGcmMulGcm_avx PROC PUBLIC FRAME
      USES_GPR    rsi,rdi
      USES_XMM xmm15
      COMP_ABI 2
pHash    equ  rdi         ; (rdi) pointer to the Hash value
pHKey    equ  rsi         ; (rsi) pointer to the (hkey<<1) value

   movdqa   xmm0, oword ptr [pHash]
   pshufb   xmm0, oword ptr SHUF_CONST
   movdqa   xmm1, oword ptr [pHKey]

   sse_clmul_gcm  xmm0, xmm1, xmm2, xmm3, xmm4  ; xmm0 holds Hash*HKey mod poly

   pshufb   xmm0, oword ptr SHUF_CONST
   movdqa   oword ptr [pHash], xmm0
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
ALIGN IPP_ALIGN_FACTOR
IPPASM AesGcmAuth_avx PROC PUBLIC FRAME
      USES_GPR    rsi,rdi
      USES_XMM xmm15
      COMP_ABI 5
pHash    equ   rdi         ; (rdi) pointer to the Hash value
pSrc     equ   rsi         ; (rsi) pointer to the input data
len      equ   rdx         ; (rdx) length of data (multiplr by AES_DATA_BLOCK)
pHKey    equ   rcx         ; (rcx) pointer to the (hkey<<1) value
BYTES_PER_BLK = (16)
  ...
  ALIGN IPP_ALIGN_FACTOR
auth_loop:
   movdqu   xmm2, oword ptr [pSrc]  ; src[]
   pshufb   xmm2, oword ptr SHUF_CONST
...
;;
IPPASM AesGcmEnc_avx PROC PUBLIC FRAME
      USES_GPR rsi,rdi,rbx
      LOCAL_FRAME = (8*16)
      USES_XMM xmm6,xmm7,xmm8,xmm9,xmm10,xmm11,xmm12,xmm13,xmm14,xmm15
      COMP_ABI 10
pDst           equ   rdi         ; pointer to the encrypted data
pSrc           equ   rsi         ; pointer to the plane data
len            equ   rdx         ; data length in bytes (multiple by BYTES_PER_BLK)
cipher         equ   rcx         ; ciper function (don't need in fact)
nr             equ   r8d         ; number of cipher's rounds
pRKey          equ   r9          ; pointer to the cipher's round keys
pGhash         equ   [rsp+ARG_7] ; pointer to the Hash value
pCtrValue      equ   [rsp+ARG_8] ; pointer to the counter value
pEncCtrValue   equ   [rsp+ARG_9] ; pointer to the encrypted counter
pPrecomData    equ   [rsp+ARG_10]; pointer to the precomputed data
...
HKeyKaratsuba  = (GHASH3+sizeof_oword_)
   ...
   movdqa   xmm4,oword ptr SHUF_CONST

   movdqu   xmm0, oword ptr[rax]       ; counter value
   movdqu   xmm1, oword ptr[rbx]       ; encrypted counter value
   movdqu   xmm2, oword ptr[rcx]       ; hash value

   pshufb   xmm0, xmm4                 ; convert counter and
   movdqa   oword ptr [rsp+CNT], xmm0  ; store into the stack
   movdqa   oword ptr [rsp+ECNT], xmm1 ; store encrypted counter into the stack

   pshufb   xmm2, xmm4                 ; convert hash value
   pxor     xmm1, xmm1
   movdqa   oword ptr [rsp+GHASH0], xmm2  ; store hash into the stack
   movdqa   oword ptr [rsp+GHASH1], xmm1  ;
   movdqa   oword ptr [rsp+GHASH2], xmm1  ;
   movdqa   oword ptr [rsp+GHASH3], xmm1  ;

   mov         rbx, [rsp+ARG_10]          ; pointer to the {hk<<1,hk^2<<1,kh^4<<1} multipliers
   movdqa      xmm10,oword ptr [rbx+sizeof_oword_*2]
   pshufd      xmm9, xmm10, 01001110b     ; xmm9 holds qword-swapped version of (HashKey^4)<<1 mod poly for Karatsuba
   pxor        xmm9, xmm10
   movdqa      oword ptr [rsp+HKeyKaratsuba], xmm9
...
blks4_loop:
   ...
   ;; ctr encryption
   ;;
   movdqa   xmm6,oword ptr INC_1
   movdqa   xmm5,oword ptr SHUF_CONST
   ...
   paddd    xmm4, xmm6
   movdqa   oword ptr [rsp+CNT], xmm4

   movdqa   xmm0, oword ptr[rcx]       ; pre-load whitening keys
   mov      r10, rcx
...
my_aesenclast  xmm4, xmm0
   ...
   movdqa      xmm0, oword ptr [rsp+ECNT]    ; load pre-calculated encrypted counter
   movdqa      oword ptr [rsp+ECNT], xmm4    ; save encrypted counter+4

   movdqu      xmm4, oword ptr[pSrc+0*BYTES_PER_BLK]  ; 4 input blocks
   movdqu      xmm5, oword ptr[pSrc+1*BYTES_PER_BLK]
   movdqu      xmm6, oword ptr[pSrc+2*BYTES_PER_BLK]
   movdqu      xmm7, oword ptr[pSrc+3*BYTES_PER_BLK]
   add         pSrc, BYTES_PER_LOOP

   pxor        xmm0, xmm4                             ; ctr encryption
   movdqu      oword ptr[pDst+0*BYTES_PER_BLK], xmm0  ; store result
   pshufb      xmm0, oword ptr SHUF_CONST             ; convert for multiplication and
   pxor        xmm0, oword ptr [rsp+GHASH0]

   pxor        xmm1, xmm5
   movdqu      oword ptr[pDst+1*BYTES_PER_BLK], xmm1
   pshufb      xmm1, oword ptr SHUF_CONST
   pxor        xmm1, oword ptr [rsp+GHASH1]

   pxor        xmm2, xmm6
   movdqu      oword ptr[pDst+2*BYTES_PER_BLK], xmm2
   pshufb      xmm2, oword ptr SHUF_CONST
   pxor        xmm2, oword ptr[rsp+GHASH2]

   pxor        xmm3, xmm7
   movdqu      oword ptr[pDst+3*BYTES_PER_BLK], xmm3
   pshufb      xmm3, oword ptr SHUF_CONST
   pxor        xmm3, oword ptr[rsp+GHASH3]
   ...
   movdqa      oword ptr [rsp+GHASH0], xmm0
   movdqa      oword ptr [rsp+GHASH1], xmm1
   movdqa      oword ptr [rsp+GHASH2], xmm2
   movdqa      oword ptr [rsp+GHASH3], xmm3

   sub         len, BYTES_PER_LOOP
   movdqa      xmm0, oword ptr [rsp+CNT]     ; next counter value
   cmp         len, BYTES_PER_LOOP
   jge         blks4_loop

combine_hash:
   movdqa      xmm8,oword ptr [rbx]                   ; hk<<1
   movdqa      xmm9,oword ptr [rbx+sizeof_oword_]     ; (hk^2)<<1
   ...
   pxor           xmm3, xmm0
   movdqa         oword ptr [rsp+GHASH0], xmm3        ; store gHash
...
single_block_proc:
  ...
  ALIGN IPP_ALIGN_FACTOR
blk_loop:
   movdqa   xmm0, oword ptr [rsp+CNT]  ; advance counter value
   movdqa   xmm1, xmm0
   paddd    xmm1, oword ptr INC_1
   movdqa   oword ptr [rsp+CNT], xmm1

   movdqa   xmm0, oword ptr[rcx]       ; pre-load whitening keys
   mov      r10, rcx

   pshufb   xmm1, oword ptr SHUF_CONST ; counter is ready to be encrypted
...
my_aesenclast  xmm1, xmm0
   ...
   movdqa      xmm0, oword ptr [rsp+ECNT]    ; load pre-calculated encrypted counter
   movdqa      oword ptr [rsp+ECNT], xmm1    ; save encrypted counter

   movdqu      xmm1, oword ptr[pSrc]         ; input block
   add         pSrc, BYTES_PER_BLK
...
;;
quit:
   movdqa   xmm0, oword ptr [rsp+CNT]     ; counter
   movdqa   xmm1, oword ptr [rsp+ECNT]    ; encrypted counter
   movdqa   xmm2, oword ptr [rsp+GHASH0]  ; hash

   mov      rax, pCtrValue             ; address of the counter
   mov      rbx, pEncCtrValue          ; address of the encrypted counter
   mov      rcx, pGhash                ; address of hash value

   pshufb   xmm0, oword ptr SHUF_CONST ; convert counter back and
   movdqu   oword ptr [rax], xmm0      ; store counter into the context

   movdqu   oword ptr [rbx], xmm1      ; store encrypted counter into the context

   pshufb   xmm2, oword ptr SHUF_CONST ; convert hach value back
   movdqu   oword ptr [rcx], xmm2      ; store hash into the context
...
;;
IPPASM AesGcmDec_avx PROC PUBLIC FRAME
      USES_GPR rsi,rdi,rbx
      LOCAL_FRAME = (8*16)
      USES_XMM xmm6,xmm7,xmm8,xmm9,xmm10,xmm11,xmm12,xmm13,xmm14,xmm15
      COMP_ABI 10
...
blks4_loop:
   ...
   paddd    xmm1, oword ptr INC_1
   movdqa   xmm2, xmm1                 ; counter+2
   paddd    xmm2, oword ptr INC_1
   movdqa   xmm3, xmm2                 ; counter+3
   paddd    xmm3, oword ptr INC_1
   movdqa   xmm4, xmm3                 ; counter+4
   paddd    xmm4, oword ptr INC_1
   movdqa   oword ptr [rsp+CNT], xmm4
...
my_aesenclast  xmm4, xmm0
   ...
   pxor        xmm0, xmm4                             ; ctr encryption
   movdqu      oword ptr[pDst+0*BYTES_PER_BLK], xmm0  ; store result
   pshufb      xmm4, oword ptr SHUF_CONST             ; convert for multiplication and
   pxor        xmm4, oword ptr [rsp+GHASH0]

   pxor        xmm1, xmm5
   movdqu      oword ptr[pDst+1*BYTES_PER_BLK], xmm1
   pshufb      xmm5, oword ptr SHUF_CONST
   pxor        xmm5, oword ptr [rsp+GHASH1]

   pxor        xmm2, xmm6
   movdqu      oword ptr[pDst+2*BYTES_PER_BLK], xmm2
   pshufb      xmm6, oword ptr SHUF_CONST
   pxor        xmm6, oword ptr [rsp+GHASH2]

   pxor        xmm3, xmm7
   movdqu      oword ptr[pDst+3*BYTES_PER_BLK], xmm3
   pshufb      xmm7, oword ptr SHUF_CONST
   pxor        xmm7, oword ptr [rsp+GHASH3]
   ...
   movdqa      oword ptr [rsp+GHASH0], xmm4
   movdqa      oword ptr [rsp+GHASH1], xmm5
   movdqa      oword ptr [rsp+GHASH2], xmm6
   movdqa      oword ptr [rsp+GHASH3], xmm7
...
combine_hash:
   ...
   pxor           xmm7, xmm4
   movdqa         oword ptr [rsp+GHASH0], xmm7        ; store gHash
...
my_aesenclast  xmm1, xmm0
   ...
   pshufb      xmm1, oword ptr SHUF_CONST
   pxor        xmm1, oword ptr [rsp+GHASH0]
   movdqa      xmm0, oword ptr [rbx]
   sse_clmul_gcm   xmm1, xmm0, xmm2, xmm3, xmm4 ; update hash value
   movdqa      oword ptr [rsp+GHASH0], xmm1
