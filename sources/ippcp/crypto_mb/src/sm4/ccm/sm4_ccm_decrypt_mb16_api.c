...
DLL_PUBLIC
mbx_status16
mbx_sm4_ccm_decrypt_mb16(int8u *pa_out[SM4_LINES], const int8u *const pa_in[SM4_LINES], const int in_len[SM4_LINES], SM4_CCM_CTX_mb16 *p_context)
{
    int buf_no;
   ...
   if (sm4_ccm_update_aad != SM4_CCM_CONTEXT_STATE(p_context) &&
    ...
    if (MBX_IS_ANY_OK_STS16(status))
        sm4_ccm_decrypt_mb16(pa_out, pa_in, in_len, mb_mask, p_context);
