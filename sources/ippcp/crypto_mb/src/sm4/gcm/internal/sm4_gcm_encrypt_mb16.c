...
/*
// This function performs encryption of given data and updates ghash with encrypted data.
// Function returns mask where bit is set to 1 if length of given data for buffer is overflowed.
*/

__mmask16 sm4_gcm_encrypt_mb16(int8u *pa_out[SM4_LINES],
                               ...
                               SM4_GCM_CTX_mb16 *p_context)
{
   ...
   SM4_GCM_CONTEXT_STATE(p_context) = sm4_gcm_enc;
   ...
   */
   ...
   for (int i = 0; i < 4; i++) {
      __m512i len_updade  = maskz_expandloadu_epi32(0x1111, (void *)(in_len_rearranged + i * 4)); /* Load txt len to high part of _m512iistry */
   ...
   sm4_gcm_update_ghash_full_blocks_mb16(
      SM4_GCM_CONTEXT_GHASH(p_context), (const int8u **)loc_pa_out, &loc_in_len, SM4_GCM_CONTEXT_HASHKEY(p_context), mb_mask);

   if (cmp_epi32_mask(loc_in_len, setzero(), _MM_CMPINT_EQ) != 0xFFFF) {
      /* Update intermediate ghash value with partial blocks of encrypted data */
      sm4_gcm_update_ghash_partial_blocks_mb16(
         SM4_GCM_CONTEXT_GHASH(p_context), (const int8u **)loc_pa_out, &loc_in_len, SM4_GCM_CONTEXT_HASHKEY(p_context)[0], mb_mask);
