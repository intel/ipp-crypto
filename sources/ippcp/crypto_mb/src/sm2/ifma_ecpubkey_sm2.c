...
*/
DLL_PUBLIC
mbx_status mbx_sm2_ecpublic_key_ssl_mb8(BIGNUM* pa_pubx[8],
                                        BIGNUM* pa_puby[8],
                                        BIGNUM* pa_pubz[8],
                                         ...
                                         int8u* pBuffer)
{
   ...
   */
   ...
   status |= MBX_SET_STS_BY_MASK(status, is_zero(scalarz, PSM2_LEN64+1), MBX_STATUS_MISMATCH_PARAM_ERR);
   ...
   /* compute public keys */
   MB_FUNC_NAME(ifma_ec_sm2_mul_pointbase_)(&P, scalarz);
   /* clear copy of the secret keys */
   MB_FUNC_NAME(zero_)((int64u (*)[8])scalarz, sizeof(scalarz)/sizeof(U64));

   if(!use_jproj_coords)
      MB_FUNC_NAME(get_sm2_ec_affine_coords_)(P.X, P.Y, &P);

   /* convert P coordinates to regular domain */
   MB_FUNC_NAME(ifma_frommont52_psm2_)(P.X, P.X);
   MB_FUNC_NAME(ifma_frommont52_psm2_)(P.Y, P.Y);
   if(use_jproj_coords)
...
DLL_PUBLIC
mbx_status mbx_sm2_ecpublic_key_mb8(int64u* pa_pubx[8],
                                    int64u* pa_puby[8],
                                    int64u* pa_pubz[8],
                                     ...
                                     int8u* pBuffer)
{
   ...
   */
   ...
   /* store result */
   ifma_mb8_to_BNU(pa_pubx, (const int64u (*)[8])P.X, PSM2_BITSIZE);
   ifma_mb8_to_BNU(pa_puby, (const int64u (*)[8])P.Y, PSM2_BITSIZE);
   if(use_jproj_coords)
      ifma_mb8_to_BNU(pa_pubz, (const int64u (*)[8])P.Z, PSM2_BITSIZE);
