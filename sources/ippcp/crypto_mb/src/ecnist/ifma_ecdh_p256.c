...
//
// Note:
// input party's public key is represented either
                                     ...
                                     int8u* pBuffer)
{
   ...
   */
   if( MBX_IS_ANY_OK_STS(status) ) {
      ...
      ifma_BN_transpose_copy((int64u (*)[])secretz, (const BIGNUM**)pa_skey, P256_BITSIZE);
      ...
      MB_FUNC_NAME(zero_)((int64u (*)[])secretz, sizeof(secretz)/sizeof(U64));
      ...
      /* to regular domain */
      ifma_frommont52_p256_mb8(R.X, R.X);

      /* store result */
      ifma_mb8_to_HexStr8(pa_shared_key, (const int64u (*)[])R.X, P256_BITSIZE);

      /* clear computed shared keys */
      MB_FUNC_NAME(zero_)((int64u (*)[])&R, sizeof(R)/sizeof(U64));
   }
   ...
   */
   if( MBX_IS_ANY_OK_STS(status) ) {
      ...
      if(use_jproj_coords)
         ...
         MB_FUNC_NAME(mov_FE256_)(P.Z, (U64*)ones);
      /* convert to Montgomery */
      ifma_tomont52_p256_mb8(P.X, P.X);
      ifma_tomont52_p256_mb8(P.Y, P.Y);
      ifma_tomont52_p256_mb8(P.Z, P.Z);
