...
fips_test_status fips_selftest_mbx_nistp521_ecdsa_sign_setup_complete_mb8(void) {
  ...
  /* k key inversion */
  int8u inv_k[MBX_LANES][NISTP521_MAX_DATA_BYTE_LEN];
  /* precomputed r signature component */
  int8u precomp_r[MBX_LANES][NISTP521_MAX_DATA_BYTE_LEN];
  /* output signature */
  ...
  mbx_status sts;
  // sign setup
  sts = mbx_nistp521_ecdsa_sign_setup_mb8(pa_inv_k, pa_precomp_r, pa_prv_k, NULL);
  if (sts != expected_status_mb8) {
    test_result = MBX_ALGO_SELFTEST_BAD_ARGS_ERR;
    return test_result;
  }
  // sign complete
  sts = mbx_nistp521_ecdsa_sign_complete_mb8(pa_sign_r, pa_sign_s, pa_pub_msg_digest,
...
fips_test_status fips_selftest_mbx_nistp521_ecdsa_sign_setup_complete_ssl_mb8(void) {
  ...
  /* test functions */
  // sign setup
  sts = mbx_nistp521_ecdsa_sign_setup_ssl_mb8(pa_inv_k, pa_precomp_r, pa_prv_k, NULL);
  if (sts != expected_status_mb8) {
    test_result = MBX_ALGO_SELFTEST_BAD_ARGS_ERR;
    MEM_FREE(pa_inv_k, pa_precomp_r, BN_d, BN_k)
    return test_result;
  ...
  // sign complete
  sts = mbx_nistp521_ecdsa_sign_complete_ssl_mb8(pa_sign_r, pa_sign_s, pa_pub_msg_digest,
