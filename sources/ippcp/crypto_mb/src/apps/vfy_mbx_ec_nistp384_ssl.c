...
static int test_mbx_nistp384_ecpublic_key_ssl(BN_CTX* ctx, int coord_mode)
{
   int res = 1;
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      // ifma pub key
      if(J_COORDS == coord_mode)
         mbx_nistp384_ecpublic_key_ssl_mb8(ifma_bnrx, ifma_bnry, ifma_bnrz, (const BIGNUM **)prvA, 0);
      else
         mbx_nistp384_ecpublic_key_ssl_mb8(ifma_bnrx, ifma_bnry, 0, (const BIGNUM **)prvA, 0);
...
static int test_mbx_nistp384_ecdh_ssl(BN_CTX* ctx, int coord_mode)
{
   int res = 1;
   const int num_tests = MAX_TEST_ROUND;
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      // ifma shared key
      if(J_COORDS == coord_mode) {
         mbx_nistp384_ecdh_ssl_mb8(pa_ifma_sharedAB, (const BIGNUM**)pa_ephA, (const BIGNUM**)pa_pubBx, (const BIGNUM**)pa_pubBy, (const BIGNUM**)pa_pubBz, 0);
         mbx_nistp384_ecdh_ssl_mb8(pa_ifma_sharedBA, (const BIGNUM**)pa_ephB, (const BIGNUM**)pa_pubAx, (const BIGNUM**)pa_pubAy, (const BIGNUM**)pa_pubAz, 0);
      }
      else {
         mbx_nistp384_ecdh_ssl_mb8(pa_ifma_sharedAB, (const BIGNUM**)pa_ephA, (const BIGNUM**)pa_pubBx, (const BIGNUM**)pa_pubBy, NULL, 0);
         mbx_nistp384_ecdh_ssl_mb8(pa_ifma_sharedBA, (const BIGNUM**)pa_ephB, (const BIGNUM**)pa_pubAx, (const BIGNUM**)pa_pubAy, NULL, 0);
      }
...
static int test_mbx_nistp384_ecdsa_sign_ssl(BN_CTX* ctx)
{
   // get order
   BIGNUM *order = BN_new();
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      for(int i=0; i<8; i++) {
         ...
         if(n==0)
            ...
            BN_rand(bn_msg, 384, -1, 0);  // top and bottom bits could be zero
      ...
      mbx_nistp384_ecdsa_sign_ssl_mb8(pa_sign_r, pa_sign_s,
                                      (const int8u**)pa_msg,
                                      (const BIGNUM**)bn_k,
...
static int test_mbx_nistp384_ecdsa_sign2_ssl(BN_CTX* ctx)
{
   // get order
   BIGNUM *order = BN_new();
   EC_GROUP_get_order(EC, order, 0);
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      mbx_nistp384_ecdsa_sign_setup_ssl_mb8(pabn_inv_eph_key,
                                            pabn_sign_rp,
                                            (const BIGNUM**)bn_k,
                                            0);
      memset(sign_r, 0, sizeof(sign_r));
      memset(sign_s, 0, sizeof(sign_s));
      mbx_nistp384_ecdsa_sign_complete_ssl_mb8(pa_sign_r, pa_sign_s,
                                               (const int8u**)pa_msg,
...
int main(void)
{
   char seed_info[] = "tests_of_mbx_ec_nistp384...ssl";
   ...
   printf("mbx_nistp384_ecpublic_key_ssl_mb8(projective_coordinates): \n");
   if(1 != test_mbx_nistp384_ecpublic_key_ssl(ctx, J_COORDS)) goto err;
   printf("mbx_nistp384_ecpublic_key_ssl_mb8(affine_coordinates): \n");
   if(1 != test_mbx_nistp384_ecpublic_key_ssl(ctx, A_COORDS)) goto err;

   printf("mbx_nistp384_ecdh_ssl_mb8(projective_coordinates): \n");
   if(1 != test_mbx_nistp384_ecdh_ssl(ctx, J_COORDS)) goto err;
   printf("mbx_nistp384_ecdh_ssl_mb8(affine_coordinates): \n");
   if(1 != test_mbx_nistp384_ecdh_ssl(ctx, A_COORDS)) goto err;

   printf("mbx_nistp384_ecdsa_sign_ssl_mb8(): \n");
   if(1 != test_mbx_nistp384_ecdsa_sign_ssl(ctx)) goto err;

   printf("mbx_nistp384_ecdsa_sign_setup/complete_ssl_mb8(): \n");
   if(1 != test_mbx_nistp384_ecdsa_sign2_ssl(ctx)) goto err;
