...
#define LEN64 (NUM_OF_DIGS(GF256_BITLEN, 64))
...
static int test_mbx_nistp256_ecpublic_key(BN_CTX* ctx, int coord_mode)
{
   int res = 1;
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      // ifma pub key
      if(J_COORDS == coord_mode)
         mbx_nistp256_ecpublic_key_mb8((int64u**)pa_ifma_pubAx, (int64u**)pa_ifma_pubAy, (int64u**)pa_ifma_pubAz, (const int64u**)pa_prvA, 0);
      else
         mbx_nistp256_ecpublic_key_mb8((int64u**)pa_ifma_pubAx, (int64u**)pa_ifma_pubAy, 0, (const int64u**)pa_prvA, 0);
...
static int test_mbx_nistp256_ecdh(BN_CTX* ctx, int coord_mode)
{
   int res = 1;
   const int num_tests = MAX_TEST_ROUND;
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      // ifma shared key
      if(J_COORDS == coord_mode) {
         mbx_nistp256_ecdh_mb8(pa_ifma_sharedAB, (const int64u**)pa_prvA, (const int64u**)pa_pubBx, (const int64u**)pa_pubBy, (const int64u**)pa_pubBz, 0);
         mbx_nistp256_ecdh_mb8(pa_ifma_sharedBA, (const int64u**)pa_prvB, (const int64u**)pa_pubAx, (const int64u**)pa_pubAy, (const int64u**)pa_pubAz, 0);
      }
      else {
         mbx_nistp256_ecdh_mb8(pa_ifma_sharedAB, (const int64u**)pa_prvA, (const int64u**)pa_pubBx, (const int64u**)pa_pubBy, NULL, 0);
         mbx_nistp256_ecdh_mb8(pa_ifma_sharedBA, (const int64u**)pa_prvB, (const int64u**)pa_pubAx, (const int64u**)pa_pubAy, NULL, 0);
      }
...
static int test_mbx_nistp256_ecdsa_sign(BN_CTX* ctx)
{
   // get order
   BIGNUM *order = BN_new();
   ...
   int64u eph_skey[8][256/64];
   int64u reg_skey[8][256/64];
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      mbx_nistp256_ecdsa_sign_mb8(pa_sign_r, pa_sign_s,
                                  (const int8u**)pa_msg,
...
static int test_mbx_nistp256_ecdsa_sign2(BN_CTX* ctx)
{
   // get order
   BIGNUM *order = BN_new();
   EC_GROUP_get_order(EC, order, 0);
   ...
   for(int n=0; n<num_tests && res; n++) {
      ...
      memset(sign_r, 0, sizeof(sign_r));
      memset(sign_s, 0, sizeof(sign_s));
      mbx_nistp256_ecdsa_sign_setup_mb8(pa_inv_eph_skey,
                                        pa_sign_rp,
                                        (const int64u**)pa_eph_skey,
                                        0);
      mbx_nistp256_ecdsa_sign_complete_mb8(pa_sign_r, pa_sign_s,
                                           (const int8u**)pa_msg,
...
int main(void)
{
   char seed_info[] = "tests_of_mbx_ec_nistp256_cp ...";
   ...
   printf("mbx_nistp256_ecpublic_key_mb8(projective_coordinates): \n");
   if(1 != test_mbx_nistp256_ecpublic_key(ctx, J_COORDS)) goto err;
   printf("mbx_nistp256_ecpublic_key_mb8(affine_coordinates): \n");
   if(1 != test_mbx_nistp256_ecpublic_key(ctx, A_COORDS)) goto err;

   printf("mbx_nistp256_ecdh_mb8(projective_coordinates): \n");
   if(1 != test_mbx_nistp256_ecdh(ctx, J_COORDS)) goto err;
   printf("mbx_nistp256_ecdh_mb8(affine_coordinates): \n");
   if(1 != test_mbx_nistp256_ecdh(ctx, A_COORDS)) goto err;

   printf("mbx_nistp256_ecdsa_sign_mb8(): \n");
   if(1 != test_mbx_nistp256_ecdsa_sign(ctx)) goto err;

   printf("mbx_nistp256_ecdsa_sign_setup/complete_mb8(): \n");
   if(1 != test_mbx_nistp256_ecdsa_sign2(ctx)) goto err;
