...
mbx_status sm3_update_mb8(const int8u* msg_pa[8], int len[8], SM3_CTX_mb8* p_state)
{
    ...
    __mmask8 mb_mask = _mm256_cmp_epi32_mask(loc_len, M256(&zero_buffer), 4);

    /* generate mask based on msg_pa[]. Don't process the data from i buffer if in msg_pa[i] == 0 or len[i] == 0 */
    mb_mask &= _mm512_cmp_epi64_mask(_mm512_loadu_si512(msg_pa), zero_buffer, 4);
    ...
    if (mb_mask) {
        ...
        __mmask8 processed_mask = _mm256_cmp_epi32_mask(idx, M256(&zero_buffer), 4);
        ...
        if (processed_mask) {
            ...
            processed_mask = _mm256_cmp_epi32_mask(_mm256_sub_epi32(loc_len, tmp), M256(&zero_buffer), 1);
            ...
            processed_mask = _mm256_cmp_epi32_mask(idx, _mm256_set1_epi32(SM3_MSG_BLOCK_SIZE), 0);
            ...
            if (processed_mask) {
                ...
                idx = _mm256_mask_set1_epi32(idx, ~_mm256_cmp_epi32_mask(proc_len, M256(&zero_buffer), 2), 0);
            }
        }

        /* main message part processing */
        proc_len = _mm256_and_si256(loc_len, _mm256_set1_epi32(-SM3_MSG_BLOCK_SIZE));
        processed_mask = _mm256_cmp_epi32_mask(proc_len, M256(&zero_buffer), 5);
        ...
        processed_mask = _mm256_cmp_epi32_mask(loc_len, M256(&zero_buffer), 6);
