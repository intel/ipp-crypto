...
IPP_OWN_DEFN(int, gfec_SharedSecretDH_sm2_avx512,
             (IppsGFpECPoint * pR,
    ...
    FIX_BNU(pScalar, scalarLen);
    {
        ...
        __ALIGN64 PSM2_POINT_IFMA P52, R52;
        ...
        gesm2_mul(&R52, &P52, (Ipp8u*)pExtendedScalar, orderBits);

        /* Check if the point - point to infinity */
        const mask8 is_zero_z = FESM2_IS_ZERO(R52.z);
        int finite_point      = ((mask8)0xFF != is_zero_z);

        /* Get X affine coordinate */
        gesm2_to_affine(/* x = */ &(R52.x), /* y = */ NULL, /* a = */ &R52);
