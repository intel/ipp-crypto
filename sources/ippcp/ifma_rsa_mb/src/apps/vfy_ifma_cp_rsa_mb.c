...
static int cp_test_ifma_rsa(int rsaBitsize, int flag)
{
   #define NUM_OF_TESTS (10) //(1000)
   int ret = 1;
   int slot, ntest;
   ...
   for(ntest=0; ntest<NUM_OF_TESTS && ret; ntest++) {
      ...
      if(ret) {
         // ifma cp rsa
         memset(ifma_encbuf_mb8, 0, sizeof(ifma_encbuf_mb8));
         ifma_cp_rsa52_public_mb8((const int8u **)ifma_ptxt_mb8, ifma_enc_mb8,
                               (const int64u **)bnu_n_mb8,
                               rsaBitsize,
							   NULL, NULL);
         // comparison
         ret = (0 == mb8_diff(ctxt, ifma_enc_mb8, len8));
         if(ret) printf("cp enc  %d passed\n", ntest);
         else {
            int len;
      ...
      if (ret && (flag & RSA_DEC2)) {
         memset(ifma_decbuf_mb8, 0, sizeof(ifma_decbuf_mb8));
         ifma_cp_rsa52_private_mb8((const int8u **)ifma_enc_mb8, ifma_dec_mb8,
         ...
         if (ret) printf("cp dec2 %d passed\n", ntest);
         else {
            int len;
            RSA_get0_key(rsa[err_buf_no], &bn_n, NULL, NULL);
      ...
      if (ret && (flag & RSA_DEC5)) {
         memset(ifma_decbuf_mb8, 0, sizeof(ifma_decbuf_mb8));
         ifma_cp_rsa52_private_ctr_mb8((const int8u **)ifma_enc_mb8, ifma_dec_mb8,
         ...
         // comparison
         ret = (0 == mb8_diff(ptxt, ifma_dec_mb8, len8));
         if (ret) printf("cp dec5 %d passed\n", ntest);
         else {
            int len;
...
int main(void)
{
   char seed_info[] = "ifma_tests_of_low_level_arithmetic ...";
   RAND_seed(seed_info, sizeof(seed_info));

   // vesrsion
   printf("%s\n", ifma_getversion()->strVersion);

   printf("cp ifma_rsa52_mb8(1024):\n");
   if (0 == cp_test_ifma_rsa(1024, RSA_ENC|RSA_DEC2|RSA_DEC5)) printf("failed\n");
   else printf("passed\n");

   printf("cp ifma_rsa52_mb8(2048):\n");
   if (0 == cp_test_ifma_rsa(2048, RSA_ENC|RSA_DEC2|RSA_DEC5)) printf("failed\n");
   else printf("passed\n");

   printf("cp ifma_rsa52_mb8(3072):\n");
   if (0 == cp_test_ifma_rsa(3072, RSA_ENC|RSA_DEC2|RSA_DEC5)) printf("failed\n");
   else printf("passed\n");

   printf("cp ifma_rsa52_mb8(4096):\n");
   if (0 == cp_test_ifma_rsa(4096, RSA_ENC|RSA_DEC2|RSA_DEC5)) printf("failed\n");
   else printf("passed\n");
