...
IPPASM DecryptCFB_RIJ128pipe_AES_NI PROC NEAR C PUBLIC \
USES  esi edi ebx,\
...
cfbBlks:DWORD,\    ; length of stream in cfbSize
...
blks_loop:
   ...
   lea      ebx, [edx+edx*2]
   movdqu   xmm0, oword ptr [esp]      ; get encoded blocks
   movdqu   xmm1, oword ptr [esp+edx]
   movdqu   xmm2, oword ptr [esp+edx*2]
   movdqu   xmm3, oword ptr [esp+ebx]
...
cipher_loop:
   ...
   movdqu      xmm4, oword ptr[esp+16]
   movdqu      xmm5, oword ptr[esp+16+edx]
   movdqu      xmm6, oword ptr[esp+16+edx*2]
   movdqu      xmm7, oword ptr[esp+16+ebx]

   pxor        xmm0, xmm4              ; xor src
   movdqu      oword ptr[esp+5*16],xmm0;and store into the stack
   pxor        xmm1, xmm5
   movdqu      oword ptr[esp+5*16+edx], xmm1
   pxor        xmm2, xmm6
   movdqu      oword ptr[esp+5*16+edx*2], xmm2
   pxor        xmm3, xmm7
   movdqu      oword ptr[esp+5*16+ebx], xmm3

   lea         eax, [edx*BLKS_PER_LOOP]
   COPY_32U    edi, <esp+5*16>, eax, ebx  ; move 4 blocks to output

   movdqu      xmm0, oword ptr[esp+eax]   ; update IV
...
short_input:
   ...
   xor      eax, eax                   ; index
single_blk_loop:
   movdqu   xmm0, oword ptr[esp+eax]   ; get encoded block

   pxor     xmm0, oword ptr [ecx]      ; whitening

   cmp      nr, 12                     ; switch according to number of rounds
   jl       key_128_s
   jz       key_192_s

key_256_s:
   aesenc     xmm0, oword ptr[esi-4*4*SC]
   aesenc     xmm0, oword ptr[esi-3*4*SC]
key_192_s:
   aesenc     xmm0, oword ptr[esi-2*4*SC]
   aesenc     xmm0, oword ptr[esi-1*4*SC]
key_128_s:
   aesenc     xmm0, oword ptr[esi+0*4*SC]
   aesenc     xmm0, oword ptr[esi+1*4*SC]
   aesenc     xmm0, oword ptr[esi+2*4*SC]
   aesenc     xmm0, oword ptr[esi+3*4*SC]
   aesenc     xmm0, oword ptr[esi+4*4*SC]
   aesenc     xmm0, oword ptr[esi+5*4*SC]
   aesenc     xmm0, oword ptr[esi+6*4*SC]
   aesenc     xmm0, oword ptr[esi+7*4*SC]
   aesenc     xmm0, oword ptr[esi+8*4*SC]
   aesenclast xmm0, oword ptr[esi+9*4*SC]

   movdqu   xmm1, oword ptr[esp+eax+16]   ; get input block from the stack
   pxor     xmm0, xmm1                    ; xor src
   movdqu   oword ptr[esp+5*16+eax], xmm0 ; and save output
...
ALIGN IPP_ALIGN_FACTOR
IPPASM DecryptCFB32_RIJ128pipe_AES_NI PROC NEAR C PUBLIC \
USES esi edi ebx,\
pInpBlk:PTR DWORD,\    ; input  block address
pOutBlk:PTR DWORD,\    ; output block address
nr:DWORD,\    ; number of rounds
pKey:PTR DWORD,\    ; key material address
cfbBlks:DWORD,\    ; length of stream in cfbSize
cfbSize:DWORD,\    ; cfb blk size (4 bytes multible)
pIV:PTR DWORD     ; IV
...
;;
ALIGN IPP_ALIGN_FACTOR
IPPASM DecryptCFB128_RIJ128pipe_AES_NI PROC NEAR C PUBLIC \
USES esi edi ebx,\
pInpBlk:PTR DWORD,\    ; input  block address
pOutBlk:PTR DWORD,\    ; output block address
nr:DWORD,\    ; number of rounds
...
;;
blks_loop:
   movdqa   xmm7, oword ptr[ecx]       ; keys for whitening
   lea      ebx, [ecx+16]              ; pointer to the round's key material

   movdqu   xmm1, oword ptr [esi+0*BYTES_PER_BLK] ; get another encoded cblocks
   movdqu   xmm2, oword ptr [esi+1*BYTES_PER_BLK]
   movdqu   xmm3, oword ptr [esi+2*BYTES_PER_BLK]
   ...
   movdqa   xmm7, oword ptr[ebx]       ; pre load operation's keys
   add      ebx, 16
...
cipher_loop:
   ...
   movdqu      xmm4, oword ptr[esi+0*BYTES_PER_BLK]
...
key_128_s:
   ...
   movdqu      xmm1, oword ptr[esi]       ; input block from the stream
   add         esi, BYTES_PER_BLK

   pxor        xmm0, xmm1                 ; xor src
   movdqu      oword ptr[edi], xmm0       ; and save output
