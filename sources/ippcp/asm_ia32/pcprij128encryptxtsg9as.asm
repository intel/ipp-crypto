...
IPPASM cpAESEncryptXTS_AES_NI PROC NEAR C PUBLIC \
USES esi edi ebx,\
...
_nr      = _tweak3+sizeof(oword)       ; # rounds
...
short_input:
   ...
   mov      eax, dword ptr[esp+_nr]    ; bottom of the keys
...
single_blk_loop_ep:
   ...
   pxor     xmm1, oword ptr[ecx]       ; AES whitening

   cmp      eax,12                     ; switch according to number of rounds
   jl       key_128_s

key_256_s:
   aesenc   xmm1,oword ptr[ebx-4*4*SC]
   aesenc   xmm1,oword ptr[ebx-3*4*SC]
   aesenc   xmm1,oword ptr[ebx-2*4*SC]
   aesenc   xmm1,oword ptr[ebx-1*4*SC]
key_128_s:
   aesenc   xmm1,oword ptr[ebx+0*4*SC]
   aesenc   xmm1,oword ptr[ebx+1*4*SC]
   aesenc   xmm1,oword ptr[ebx+2*4*SC]
   aesenc   xmm1,oword ptr[ebx+3*4*SC]
   aesenc   xmm1,oword ptr[ebx+4*4*SC]
   aesenc   xmm1,oword ptr[ebx+5*4*SC]
   aesenc   xmm1,oword ptr[ebx+6*4*SC]
   aesenc   xmm1,oword ptr[ebx+7*4*SC]
   aesenc   xmm1,oword ptr[ebx+8*4*SC]
   aesenclast  xmm1,oword ptr[ebx+9*4*SC]
