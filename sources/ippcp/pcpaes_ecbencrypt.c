...
static void cpEncryptAES_ecb_dispatch(const Ipp8u *pSrc, Ipp8u *pDst, int len, const IppsAESSpec *pCtx)
{
                                     ...
                                     const IppsAESSpec* pCtx))
{
   ...
   if (AES_NOISE_LEVEL(params) > 0) {
      ...
      while (remaining_size > 0) {
         ...
         cpEncryptAES_ecb_dispatch(pSrc, pDst, chunk_size, pCtx);
...
#endif
   {
      cpEncryptAES_ecb_dispatch(pSrc, pDst, len, pCtx);
   }
