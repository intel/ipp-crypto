...
GENERATOR_FORMAT = {
    WINDOWS: {
        WINDOWS: ':: Generates  {threading} dynamic library '
                 ...
                 + '@echo off\n\n'
                 + 'if not defined {root} echo Please, set {root} & exit /b 1\n'
                 + 'if not exist "{destination}" '
                 + 'mkdir "{destination}"\n\n'
                 + '{sub_command}'
                 + '{compiler} {cmp_args} '
                 + '/I "%{root}%\\include" "{main_file}" '
                 + '/Fo:"{main_object}"\n\n'
                 ...
                 + '{libraries} '
                 + '{exp_libs}\n\n'
                 + 'if exist "{library_file}.dll" (\n'
                 + '\techo Build dynamic library for {architecture} architecture OK\n'
                 + ') else (\n'
                 + '\techo Build dynamic library for {architecture} architecture Failed\n'
                 + ')',
